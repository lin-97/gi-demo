...
export function getTableList(data: Pagination) {
  return http.get<ApiRes<ApiListData<ApiTableItem[]>>>(`${prefix}/table/list`, data)
}

/** @desc 获取表格, 如果想要以表单数据格式发请求, 可以用以下方式定义接口 */
export function getTableListByFormData(data: Pagination) {
  return http.get<ApiRes<ApiListData<ApiTableItem[]>>>(`${prefix}/table/list`, qs.stringify(data), {
